  ...
  return <main className="min-h-screen bg-[#f7faf8]">
    ...
    <div className="section-shell py-8 sm:py-12">
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px] 2xl:grid-cols-[minmax(0,1fr)_400px] 2xl:gap-10"><div className="space-y-7">
        ...
        {operator && <section className="rounded-3xl border border-blue-100 bg-blue-50 p-6 sm:p-8"><h2 className="text-lg font-black text-navy">Туроператор: {operator.name}</h2><p className="mt-2 text-sm leading-6 text-slate-600">Цена проверена {tour.priceCheckedAt ? new Date(tour.priceCheckedAt).toLocaleString('ru-RU') : 'при последнем обновлении'}.{tour.sourcePrice && tour.sourceCurrency ? ` Исходная цена: ${new Intl.NumberFormat('ru-RU').format(tour.sourcePrice)} ${tour.sourceCurrency}.` : ''}</p>{operator.url && <a href={operator.url} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-blue-700 shadow-sm transition hover:bg-blue-100">Открыть официальный сайт {operator.name} <ExternalLink className="size-4"/></a>}</section>}
        {tour.partnerOffers && tour.partnerOffers.length > 1 && <section className="rounded-3xl bg-white p-6 sm:p-8"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.16em] text-brand-dark">Сравнение операторов</p><h2 className="mt-2 text-2xl font-black">Все предложения в одной карточке</h2></div><span className="rounded-full bg-brand/10 px-3 py-1.5 text-xs font-black text-brand-dark">Лучшая цена выбрана</span></div><div className="mt-6 divide-y divide-slate-100">{tour.partnerOffers.map((offer, index) => { const offerOperator = tourOperator(offer.source, offer.sourceUrl); return <div key={`${offer.source}:${offer.externalOfferId}`} className="flex items-center justify-between gap-4 py-4"><div><p className="font-black text-navy">{offerOperator?.name ?? offer.source}</p><p className="mt-1 text-xs text-slate-400">{offer.availability}</p>{offerOperator?.url && <a href={offerOperator.url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-black text-blue-700 hover:underline">Проверить на сайте <ExternalLink className="size-3"/></a>}</div><div className="text-right"><p className="text-lg font-black text-navy">{formatPrice(offer.price)}</p>{index === 0 && <span className="text-xs font-black text-brand-dark">Лучшая цена</span>}</div></div>})}</div></section>}
