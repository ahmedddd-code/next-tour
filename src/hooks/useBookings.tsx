...
export function BookingsProvider({ children }: { children: ReactNode }) {
  ...
  const load = useCallback(async () => {
    ...
    try { const data = await invokeSiteData({ action: admin ? 'admin_list_bookings' : 'user_list_bookings', ...(admin ? { adminPassword: ADMIN_PASSWORD } : {}) }); const next = (data.bookings as Booking[]) ?? []; setBookings(current => JSON.stringify(current) === JSON.stringify(next) ? current : next); setLoaded(true); } catch { /* Повторим автоматически. */ }
  }, [user]);
