...
export function AdminSupportChats() {
  ...
  return <section><div className="mb-6"><p className="text-xs font-black uppercase tracking-[.18em] text-brand-dark">Поддержка</p><h1 className="mt-2 text-3xl font-black tracking-[-.04em] sm:text-4xl">Чаты с клиентами</h1><p className="mt-2 text-sm text-slate-500">Открытых диалогов: {conversations.filter(item => item.status === 'open').length} · ожидают клиента: {conversations.filter(item => item.status === 'pending_close').length}</p></div>
    {conversations.length === 0 ? <div className="rounded-3xl bg-white py-20 text-center shadow-sm"><MessageCircle className="mx-auto size-12 text-slate-300"/><p className="mt-4 font-black text-navy">Сообщений пока нет</p></div> : <div className="grid min-h-[620px] overflow-hidden rounded-3xl bg-white shadow-sm lg:grid-cols-[300px_1fr]">
      ...
      {selected && <div className="flex min-h-[460px] flex-col lg:min-h-0"><header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-4"><div className="min-w-0"><h2 className="truncate font-black text-navy">{selected.contact?.name || `Клиент ${selected.id.slice(-5)}`}</h2><p className="text-xs text-slate-400">{selected.contact?.subject || (selected.status === 'open' ? 'Ожидает ответа' : selected.status === 'pending_close' ? 'Ожидает подтверждения клиента' : 'Диалог закрыт')}</p>{selected.contact && <p className="mt-1 break-all text-[11px] text-slate-500">{selected.contact.phone} · {selected.contact.email}</p>}</div><div className="flex gap-2"><button onClick={() => toggleConversationStatus(selected.id)} className="flex items-center gap-2 rounded-xl bg-mist px-3 py-2 text-xs font-black text-brand-dark"><CheckCircle2 className="size-4"/>{selected.status === 'open' ? 'Предложить закрыть' : 'Открыть снова'}</button><button onClick={() => { if (window.confirm('Удалить этот диалог?')) { deleteConversation(selected.id); setSelectedId(null); } }} className="grid size-9 place-items-center rounded-xl bg-red-50 text-red-500" aria-label="Удалить диалог"><Trash2 className="size-4"/></button></div></header>
