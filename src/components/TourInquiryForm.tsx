...
export function TourInquiryForm({ tour }: { tour: Tour }) {
  ...
  return <><aside className="rounded-3xl bg-white p-6 shadow-[0_18px_60px_rgba(7,29,52,.12)] lg:sticky lg:top-6">
    ...
    <button onClick={() => setModalOpen(true)} className="mt-4 flex h-13 w-full items-center justify-center gap-2 rounded-2xl bg-brand font-extrabold text-white transition hover:bg-brand-dark hover:shadow-lg hover:shadow-brand/20"><Phone className="size-4"/>Забронировать</button>
