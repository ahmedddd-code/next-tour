...
export function TourGallery({ images, title, isDestinationFallback = false }: { images: string[]; title: string; isDestinationFallback?: boolean }) {
  ...
  return <div className="space-y-3">
    <div className="group relative h-[clamp(240px,65vw,520px)] overflow-hidden rounded-2xl bg-slate-100 sm:rounded-3xl"><img src={optimizedImageUrl(activeImage, 1400, 82)} srcSet={optimizedImageSrcSet(activeImage, [640, 960, 1400])} sizes="(max-width: 767px) calc(100vw - 24px), 1200px" alt={isDestinationFallback ? `${title}, фотография направления` : `${title}, фото ${activeIndex + 1}`} decoding="async" onError={() => removeUnavailable(activeImage)} className="size-full object-cover"/>
      {isDestinationFallback && <span className="absolute left-3 top-3 rounded-full bg-navy/75 px-3 py-1.5 text-xs font-black text-white backdrop-blur sm:left-5 sm:top-5">Фото направления</span>}
