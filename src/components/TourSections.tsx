...
export function TourSections() {
  ...
  return <>
    ...
    <section id="destinations" className="scroll-mt-24 bg-mist py-24"><div className="section-shell"><Heading eyebrow="Куда поехать" title="Популярные направления" text="Места, в которые хочется возвращаться. Выберите настроение — детали мы берём на себя."/>
      <div className="grid auto-rows-[220px] gap-4 md:grid-cols-4 md:auto-rows-[190px]">{destinations.map(item => <Link to="/tours" key={item.name} className={`group relative overflow-hidden rounded-3xl ${item.span}`}><img src={item.image} alt={item.name} loading="lazy" className="size-full object-cover transition duration-700 group-hover:scale-110"/><div className="absolute inset-0 bg-gradient-to-t from-navy/85 via-navy/10 to-transparent"/><div className="absolute inset-x-0 bottom-0 flex items-end justify-between p-6 text-white"><div><h3 className="text-2xl font-black">{item.name}</h3><p className="mt-1 text-sm text-white/70">{item.meta}</p></div><ArrowUpRight className="transition group-hover:translate-x-1 group-hover:-translate-y-1"/></div></Link>)}</div>
    </div></section>
  </>;
}
