...
export function FloatingSupportWidget({ chatUrl = '/chat' }: Props) {
  ...
  return <div className="group fixed bottom-4 right-4 z-[9999] sm:bottom-6 sm:right-6">
    {greetingVisible && <button onClick={openSupport} className="absolute bottom-[calc(100%+12px)] right-0 w-64 rounded-2xl border border-slate-100 bg-white p-4 text-left shadow-2xl animate-[support-message_.3s_ease-out]">
    ...
    <div className="pointer-events-none absolute bottom-1/2 right-[calc(100%+14px)] hidden translate-x-5 translate-y-1/2 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100 md:block">
      <div className="flex w-64 items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-2xl">
